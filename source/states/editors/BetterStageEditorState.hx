 ...
 * - Compact tabbed panel: Object / Characters / Stage / Insert.
 * - Insert tab and Object tab pick images straight from the current mod's images/
 *   folder via a dropdown (no desktop file browser, no popup).
 * - Regular layers (sprites/squares) can be dragged directly with mouse/touch;
 *   characters (gf/dad/boyfriend) are intentionally NOT draggable, to avoid
 *   accidentally bumping them - use arrow keys or the dropdowns for those.
 */
class BetterStageEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	function addHelpScreen()
	{
		var str:Array<String> = [
			"Click and Drag - Move Layer (not characters)",
			"Right Click Drag - Pan Camera",
			"Arrow Keys - Nudge Selected (works on characters too)",
	...
	function addLayerListBox()
	{
		...
		var buttonNew:PsychUIButton = new PsychUIButton(buttonX, buttonY + 60, 'New', function() editBox.selectedName = 'Insert');
		buttonNew.cameras = [camHUD];
		buttonNew.normalStyle.bgColor = FlxColor.GREEN;
		buttonNew.normalStyle.textColor = FlxColor.WHITE;
		tab_group.add(buttonNew);
	...
	// ______________________________ UI: Tabbed Edit Panel ______________________________
	// Object / Characters / Stage / Insert - compact, matches the reference mockup.
	...
	var imageDropDown:PsychUIDropDownMenu;
	...
	var insertNameInputText:PsychUIInputText;
	var insertImageDropDown:PsychUIDropDownMenu;

	function editorUI()
	{
		editBox = new PsychUIBox(FlxG.width - 270, 20, 250, 340, ['Characters', 'Stage', 'Insert', 'Object']);
	...
	// -------- Characters tab: kept minimal, matches the mockup exactly --------
	function addCharactersTab()
	{
		...
		tab_group.add(new FlxText(objX, objY, 100, 'Player'));
		objY += 20;
		...
		objY += 45;
		tab_group.add(new FlxText(objX, objY, 100, 'Opponent'));
		objY += 20;
		...
		objY += 45;
		tab_group.add(new FlxText(objX, objY, 100, 'Girlfriend'));
		objY += 20;
		...
		objY += 45;
		hideGirlfriendCheckbox = new PsychUICheckBox(objX, objY, 'Show GF?', 100);
	...
	// -------- Stage tab: camera/zoom/save/reload, consolidated from the vanilla editor --------
	function addStageTab()
	{
		var tab_group = editBox.getTab('Stage').menu;

		var objX = 10;
		var objY = 20;

		var saveButton:PsychUIButton = new PsychUIButton(150, objY - 10, 'Save', function() saveData());
		saveButton.normalStyle.bgColor = FlxColor.GREEN;
		saveButton.normalStyle.textColor = FlxColor.WHITE;
		tab_group.add(saveButton);

		var reloadStage:PsychUIButton = new PsychUIButton(150, objY + 20, 'Reload', function()
		{
		...
		tab_group.add(new FlxText(stageDropDown.x, stageDropDown.y - 18, 60, 'Stage (already loaded):'));
	...
	// -------- Insert tab: name + image dropdown, matches the mockup's "Layer / Layer1 [v]" --------
	function addInsertTab()
	{
		var tab_group = editBox.getTab('Insert').menu;

		var objX = 10;
		var objY = 20;

		tab_group.add(new FlxText(objX, objY, 100, 'Layer'));
		objY += 20;
		insertNameInputText = new PsychUIInputText(objX, objY, 200, findUnoccupiedName(), 8);
		insertNameInputText.customFilterPattern = ~/[^a-zA-Z0-9_\-]*/g;
		tab_group.add(insertNameInputText);

		objY += 40;
		tab_group.add(new FlxText(objX, objY, 100, 'Image'));
		objY += 20;
		var images:Array<String> = getModImageList();
		insertImageDropDown = new PsychUIDropDownMenu(objX, objY, images, function(sel:Int, selected:String) {});
		tab_group.add(insertImageDropDown);

		objY += 45;
		var addImageBtn:PsychUIButton = new PsychUIButton(objX, objY, 'Add Image Layer', function()
		{
			var picked:String = insertImageDropDown.selectedLabel;
			if(picked == null || picked.length < 1)
			{
				showOutput('No image selected!', true);
				return;
			}

			var layerName:String = insertNameInputText.text;
			if(layerName == null || layerName.length < 1) layerName = findUnoccupiedName();
			for (basic in stageSprites)
				if(basic.name == layerName) { showOutput('Name "$layerName" is already in use!', true); return; }

			var meta:StageEditorMetaSprite = new StageEditorMetaSprite({type: 'sprite', scale: [1, 1], scroll: [1, 1], name: layerName}, new ModchartSprite());
			meta.image = picked;
			meta.sprite.screenCenter();
			insertMeta(meta);
			insertNameInputText.text = findUnoccupiedName();
			showOutput('Added layer "$layerName"!');
		});
		tab_group.add(addImageBtn);

		objY += 40;
		var addColorBtn:PsychUIButton = new PsychUIButton(objX, objY, 'Add Solid Color Layer', function()
		{
			var layerName:String = insertNameInputText.text;
			if(layerName == null || layerName.length < 1) layerName = findUnoccupiedName();
			for (basic in stageSprites)
			...
			var meta:StageEditorMetaSprite = new StageEditorMetaSprite({type: 'square', scale: [200, 200], name: layerName}, new ModchartSprite());
		...
		tab_group.add(addColorBtn);

		objY += 40;
		var refreshBtn:PsychUIButton = new PsychUIButton(objX, objY, 'Refresh Image List', function()
		{
			insertImageDropDown.list = getModImageList();
		});
		tab_group.add(refreshBtn);
	}

	// -------- Object tab: edit an existing (non-character) layer --------
	function addObjectTab()
	{
		...
		objY += 40;
		tab_group.add(new FlxText(objX, objY - 18, 100, 'Image:'));
		imageDropDown = new PsychUIDropDownMenu(objX, objY, getModImageList(), function(sel:Int, selected:String) {
			var spr = getSelected();
			if(spr != null && selected != null && selected.length > 0)
				tryLoadImage(spr, selected);
		});
		tab_group.add(imageDropDown);
	...
	// ______________________________ Mod Image List (used by both dropdowns) ______________________________

	function getModImageList():Array<String>
	{
	...
	function updateSelectedUI()
	{
		...
		imageDropDown.selectedLabel = selected.image;
	...
	// ______________________________ Drag-to-Move (layers only, not characters) + Update Loop ______________________________
	...
	{
		var overUI:Bool = FlxG.mouse.overlaps(layerListBox, camHUD) || FlxG.mouse.overlaps(editBox, camHUD);
		...
		if(FlxG.keys.justPressed.F2 || (touchPad.buttonS.justPressed && !touchPad.buttonF.justPressed))
		{
			...
			var objs = [layerRadioGroup, layerListBox];
			for (obj in objs)
			{
		...
		// ---- Left-click drag moves the object directly under the cursor - NOT characters ----
		if(!overUI && FlxG.mouse.justPressed)
		{
			...
			while(i-- > 0)
			{
				...
				if(StageData.reservedNames.contains(spr.type)) continue; // characters aren't draggable
		...
		// ---- Arrow-key nudge - works for everything, including characters ----
		var moveX:Float = 0;
		var moveY:Float = 0;
