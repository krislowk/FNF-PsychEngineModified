...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	var previewTankman:Character;
	var previewDad:Character;
	var previewGF:Character;
	var previewDadBg:FlxSprite;
	var previewTankmanBg:FlxSprite;
	var previewGFBg:FlxSprite;
	var previewDadAnchorX:Float = 5;
	var previewDadAnchorY:Float;
	var previewTankmanAnchorX:Float;
	var previewTankmanAnchorY:Float;
	var previewGFAnchorX:Float;
	var previewGFAnchorY:Float;
	var previewDadScale:Float = 1;
	var previewTankmanScale:Float = 1;
	var previewGFScale:Float = 1;
	var draggingPreviewDad:Bool = false;
	var draggingPreviewTankman:Bool = false;
	var draggingPreviewGF:Bool = false;
	var previewDadDragOffsetX:Float = 0;
	var previewDadDragOffsetY:Float = 0;
	var previewTankmanDragOffsetX:Float = 0;
	var previewTankmanDragOffsetY:Float = 0;
	var previewGFDragOffsetX:Float = 0;
	var previewGFDragOffsetY:Float = 0;
	var previewPosSave:FlxSave;
	var previewCharactersEnabled:Bool = true;
	var previewGFEnabled:Bool = false;
	var previewCharactersButton:PsychUIButton;
	var previewGFButton:PsychUIButton;
	...
	override function update(elapsed:Float)
	{
			...
			if(PsychUIInputText.focusOn == null) //If not typing anything
			{
				...
				if(vortexEnabled && _keysPressedBuffer.contains(true))
				{
					...
					for (num => press in _keysPressedBuffer)
					{
						...
						previewNoteHit(num, num < GRID_COLUMNS_PER_PLAYER, typeSelected);
		...
		if(previewDad != null)
		{
			if(FlxG.mouse.justPressed && FlxG.mouse.overlaps(previewDad))
			{
				draggingPreviewDad = true;
				previewDadDragOffsetX = FlxG.mouse.screenX - previewDadAnchorX;
				previewDadDragOffsetY = FlxG.mouse.screenY - previewDadAnchorY;
			}
			if(draggingPreviewDad)
			{
				if(FlxG.mouse.pressed)
				{
					previewDadAnchorX = FlxG.mouse.screenX - previewDadDragOffsetX;
					previewDadAnchorY = FlxG.mouse.screenY - previewDadDragOffsetY;
				}
				else
				{
					draggingPreviewDad = false;
					savePreviewPositions();
				}
			}
			lockPreviewTransform(previewDad, previewDadScale, previewDadAnchorX, previewDadAnchorY, previewDadBg);
		}
		if(previewTankman != null)
		{
			if(FlxG.mouse.justPressed && FlxG.mouse.overlaps(previewTankman))
			{
				draggingPreviewTankman = true;
				previewTankmanDragOffsetX = FlxG.mouse.screenX - previewTankmanAnchorX;
				previewTankmanDragOffsetY = FlxG.mouse.screenY - previewTankmanAnchorY;
			}
			if(draggingPreviewTankman)
			{
				if(FlxG.mouse.pressed)
				{
					previewTankmanAnchorX = FlxG.mouse.screenX - previewTankmanDragOffsetX;
					previewTankmanAnchorY = FlxG.mouse.screenY - previewTankmanDragOffsetY;
				}
				else
				{
					draggingPreviewTankman = false;
					savePreviewPositions();
				}
			}
			lockPreviewTransform(previewTankman, previewTankmanScale, previewTankmanAnchorX, previewTankmanAnchorY, previewTankmanBg);
		}
		if(previewGF != null && previewCharactersEnabled && previewGFEnabled)
		{
			if(FlxG.mouse.justPressed && FlxG.mouse.overlaps(previewGF))
			{
				draggingPreviewGF = true;
				previewGFDragOffsetX = FlxG.mouse.screenX - previewGFAnchorX;
				previewGFDragOffsetY = FlxG.mouse.screenY - previewGFAnchorY;
			}
			if(draggingPreviewGF)
			{
				if(FlxG.mouse.pressed)
				{
					previewGFAnchorX = FlxG.mouse.screenX - previewGFDragOffsetX;
					previewGFAnchorY = FlxG.mouse.screenY - previewGFDragOffsetY;
				}
				else
				{
					draggingPreviewGF = false;
					savePreviewPositions();
				}
			}
			lockPreviewTransform(previewGF, previewGFScale, previewGFAnchorX, previewGFAnchorY, previewGFBg);
		}
						...
						else if(touch.x >= gridBg.x && touch.x < gridBg.x + gridBg.width)
						{
							...
							else if(!holdingAlt && touch.y >= gridBg.y && touch.y < gridBg.y + gridBg.height) // Add note
							{
								...
								if(noteData >= 0)
								{
									...
									var noteAdded:MetaNote = createNote(noteSetupData);
									previewNoteHit(noteData, noteData < GRID_COLUMNS_PER_PLAYER, typeSelected);
									for (num in sectionFirstNoteID...notes.length)
									{
										var note = notes[num];
										if(note.strumTime >= strumTime)
			...
			for (note in curRenderedNotes)
			{
				...
				if(note.isEvent)
				{
					if(Conductor.songPosition > note.strumTime && lastTime <= note.strumTime)
						previewPlayAnimationEvent(cast (note, EventMetaNote));
					continue;
				}
				...
				if(Conductor.songPosition > note.strumTime && lastTime <= note.strumTime)
				{
					previewNoteHit(Std.int(note.songData[1]), note.mustPress, note.noteType);
					if(canPlayHitSound)
					{
	...
	function addViewTab()
	{
		...
		if(chartEditorSave.data.previewCharactersEnabled != null)
			previewCharactersEnabled = chartEditorSave.data.previewCharactersEnabled;
		if(chartEditorSave.data.previewGFEnabled != null)
			previewGFEnabled = chartEditorSave.data.previewGFEnabled;
		updatePreviewCharactersVisibility();
		...
		btnY += 20;
		previewCharactersButton = new PsychUIButton(btnX, btnY, previewCharactersEnabled ? '  Preview Characters ON' : '  Preview Characters OFF', function()
		{
			previewCharactersEnabled = !previewCharactersEnabled;
			chartEditorSave.data.previewCharactersEnabled = previewCharactersEnabled;
			previewCharactersButton.text.text = previewCharactersEnabled ? '  Preview Characters ON' : '  Preview Characters OFF';
			updatePreviewCharactersVisibility();
		}, btnWid);
		previewCharactersButton.text.alignment = LEFT;
		tab_group.add(previewCharactersButton);

		btnY += 20;
		previewGFButton = new PsychUIButton(btnX, btnY, previewGFEnabled ? '  GF Preview Character ON' : '  GF Preview Character OFF', function()
		{
			if(!previewCharactersEnabled) return; // only togglable while Preview Characters is on
			previewGFEnabled = !previewGFEnabled;
			chartEditorSave.data.previewGFEnabled = previewGFEnabled;
			previewGFButton.text.text = previewGFEnabled ? '  GF Preview Character ON' : '  GF Preview Character OFF';
			updatePreviewCharactersVisibility();
		}, btnWid);
		previewGFButton.text.alignment = LEFT;
		tab_group.add(previewGFButton);
	}
	...
	function loadPreviewCharacters():Void
	{
		if(previewTankman != null) remove(previewTankman);
		if(previewDad != null) remove(previewDad);
		if(previewGF != null) remove(previewGF);
		if(previewDadBg != null) remove(previewDadBg);
		if(previewTankmanBg != null) remove(previewTankmanBg);
		if(previewGFBg != null) remove(previewGFBg);

		if(previewPosSave == null)
		{
			previewPosSave = new FlxSave();
			previewPosSave.bind('chartPreviewChars', CoolUtil.getSavePath());
		}

		previewDadAnchorX = 5;
		previewDadAnchorY = FlxG.height - 260;
		previewTankmanAnchorX = previewDadAnchorX + 100;
		previewTankmanAnchorY = previewDadAnchorY;
		previewGFAnchorX = previewDadAnchorX + 200;
		previewGFAnchorY = previewDadAnchorY - 20;

		if(previewPosSave.data.dadX != null) previewDadAnchorX = previewPosSave.data.dadX;
		if(previewPosSave.data.dadY != null) previewDadAnchorY = previewPosSave.data.dadY;
		if(previewPosSave.data.tankmanX != null) previewTankmanAnchorX = previewPosSave.data.tankmanX;
		if(previewPosSave.data.tankmanY != null) previewTankmanAnchorY = previewPosSave.data.tankmanY;
		if(previewPosSave.data.gfX != null) previewGFAnchorX = previewPosSave.data.gfX;
		if(previewPosSave.data.gfY != null) previewGFAnchorY = previewPosSave.data.gfY;

		// Visible black squares behind each character, so it's obvious what's draggable.
		previewDadBg = new FlxSprite().makeGraphic(150, 200, FlxColor.BLACK);
		previewDadBg.scrollFactor.set();
		previewDadBg.alpha = 0.5;
		add(previewDadBg);

		previewDad = new Character(previewDadAnchorX, previewDadAnchorY, PlayState.SONG.player2, false);
		previewDad.setGraphicSize(0, 180);
		previewDad.updateHitbox();
		previewDad.scrollFactor.set();
		previewDad.antialiasing = ClientPrefs.data.antialiasing;
		previewDadScale = previewDad.scale.x;
		previewDad.offset.set(0, 0);
		add(previewDad);

		previewTankmanBg = new FlxSprite().makeGraphic(150, 170, FlxColor.BLACK);
		previewTankmanBg.scrollFactor.set();
		previewTankmanBg.alpha = 0.5;
		add(previewTankmanBg);

		previewTankman = new Character(previewTankmanAnchorX, previewTankmanAnchorY, PlayState.SONG.player1, true);
		previewTankman.setGraphicSize(0, 140);
		previewTankman.updateHitbox();
		previewTankman.scrollFactor.set();
		previewTankman.antialiasing = ClientPrefs.data.antialiasing;
		previewTankmanScale = previewTankman.scale.x;
		previewTankman.offset.set(0, 0);
		add(previewTankman);

		previewGFBg = new FlxSprite().makeGraphic(150, 170, FlxColor.BLACK);
		previewGFBg.scrollFactor.set();
		previewGFBg.alpha = 0.5;
		add(previewGFBg);

		var gfChar:String = (PlayState.SONG.gfVersion != null) ? PlayState.SONG.gfVersion : 'gf';
		previewGF = new Character(previewGFAnchorX, previewGFAnchorY, gfChar, false);
		previewGF.setGraphicSize(0, 140);
		previewGF.updateHitbox();
		previewGF.scrollFactor.set();
		previewGF.antialiasing = ClientPrefs.data.antialiasing;
		previewGFScale = previewGF.scale.x;
		previewGF.offset.set(0, 0);
		add(previewGF);

		updatePreviewCharactersVisibility();
	}

	function updatePreviewCharactersVisibility():Void
	{
		var showMain:Bool = previewCharactersEnabled;
		var showGF:Bool = previewCharactersEnabled && previewGFEnabled;

		if(previewDad != null) previewDad.visible = showMain;
		if(previewTankman != null) previewTankman.visible = showMain;
		if(previewDadBg != null) previewDadBg.visible = showMain;
		if(previewTankmanBg != null) previewTankmanBg.visible = showMain;

		if(previewGF != null) previewGF.visible = showGF;
		if(previewGFBg != null) previewGFBg.visible = showGF;
	}

	function lockPreviewTransform(char:Character, fixedScale:Float, anchorX:Float, anchorY:Float, ?bg:FlxSprite = null):Void
	{
		char.scale.set(fixedScale, fixedScale);
		char.offset.set(0, 0);
		char.x = anchorX;
		char.y = anchorY;
	...
	function savePreviewPositions():Void
	{
		if(previewPosSave == null) return;
		previewPosSave.data.dadX = previewDadAnchorX;
		previewPosSave.data.dadY = previewDadAnchorY;
		previewPosSave.data.tankmanX = previewTankmanAnchorX;
		previewPosSave.data.tankmanY = previewTankmanAnchorY;
		previewPosSave.data.gfX = previewGFAnchorX;
		previewPosSave.data.gfY = previewGFAnchorY;
		previewPosSave.flush();
	}
	...
	function previewNoteHit(column:Int, mustPress:Bool, ?noteType:String = ''):Void
	{
		if(noteType == null) noteType = '';

		if(noteType == 'No Animation') return;

		var dirs:Array<String> = ['LEFT', 'DOWN', 'UP', 'RIGHT'];
		var dirIndex:Int = column % GRID_COLUMNS_PER_PLAYER;
		if(dirIndex < 0 || dirIndex >= dirs.length) return;

		var anim:String = 'sing' + dirs[dirIndex];

		if(noteType == 'GF Sing')
		{
			if(previewGF != null && previewCharactersEnabled && previewGFEnabled)
			{
				previewGF.playAnim(anim, true);
				previewGF.holdTimer = 0;
				lockPreviewTransform(previewGF, previewGFScale, previewGFAnchorX, previewGFAnchorY, previewGFBg);
			}
			return;
		}

		var target:Character = mustPress ? previewTankman : previewDad;
		if(target != null)
		{
			if(noteType == 'Alt Animation')
			{
				var altAnim:String = anim + '-alt';
				if(target.animation.exists(altAnim))
					anim = altAnim;
			}
			// Unrecognized custom note types (from custom_notetypes/) fall back
			// to the normal sing animation as a best-effort preview, since their
			// real behavior is defined by Lua scripts the chart editor can't run.

			target.playAnim(anim, true);
			target.holdTimer = 0;
			if(target == previewTankman)
				lockPreviewTransform(previewTankman, previewTankmanScale, previewTankmanAnchorX, previewTankmanAnchorY, previewTankmanBg);
			else
				lockPreviewTransform(previewDad, previewDadScale, previewDadAnchorX, previewDadAnchorY, previewDadBg);
		}
	...
	function previewPlayAnimationEvent(eventNote:EventMetaNote):Void
	{
		for (subEvent in eventNote.events)
		{
			if(subEvent[0] != 'Play Animation') continue;
			var animName:String = subEvent[1];
			if(animName == null || animName.length < 1) continue;

			var target:Character = null;
			if(subEvent[2] != null)
			{
				var t:String = subEvent[2].toLowerCase().trim();
				if(t == 'dad') target = previewDad;
				else if(t == 'bf' || t == 'boyfriend') target = previewTankman;
				else if(t == 'gf' || t == 'girlfriend') target = (previewCharactersEnabled && previewGFEnabled) ? previewGF : null;
			}

			if(target != null)
			{
				target.playAnim(animName, true);
				target.holdTimer = 0;
				if(target == previewTankman)
					lockPreviewTransform(previewTankman, previewTankmanScale, previewTankmanAnchorX, previewTankmanAnchorY, previewTankmanBg);
				else if(target == previewDad)
					lockPreviewTransform(previewDad, previewDadScale, previewDadAnchorX, previewDadAnchorY, previewDadBg);
				else
					lockPreviewTransform(previewGF, previewGFScale, previewGFAnchorX, previewGFAnchorY, previewGFBg);
			}
