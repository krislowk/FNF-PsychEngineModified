...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	var previewDadDragHandle:FlxSprite;
	var previewTankmanDragHandle:FlxSprite;
	...
	override function update(elapsed:Float)
	{
		...
		if(previewDad != null)
		{
			if(previewDadDragHandle != null)
			{
				previewDadDragHandle.x = previewDadAnchorX;
				previewDadDragHandle.y = previewDadAnchorY;

				if(FlxG.mouse.justPressed && FlxG.mouse.overlaps(previewDadDragHandle))
				{
			...
			lockPreviewTransform(previewDad, previewDadScale, previewDadAnchorX, previewDadAnchorY);
		}
		if(previewTankman != null)
		{
			if(previewTankmanDragHandle != null)
			{
				previewTankmanDragHandle.x = previewTankmanAnchorX;
				previewTankmanDragHandle.y = previewTankmanAnchorY;

				if(FlxG.mouse.justPressed && FlxG.mouse.overlaps(previewTankmanDragHandle))
				{
			...
			lockPreviewTransform(previewTankman, previewTankmanScale, previewTankmanAnchorX, previewTankmanAnchorY);
		}
	...
	function loadPreviewCharacters():Void
	{
		...
		// Fixed-size invisible handles for drag detection, so the click zone
		// stays consistent no matter which character or animation is loaded
		// (character art padding/offsets vary too much to rely on for this).
		if(previewDadDragHandle != null) remove(previewDadDragHandle);
		previewDadDragHandle = new FlxSprite();
		previewDadDragHandle.makeGraphic(120, 180, FlxColor.TRANSPARENT);
		previewDadDragHandle.scrollFactor.set();
		previewDadDragHandle.x = previewDadAnchorX;
		previewDadDragHandle.y = previewDadAnchorY;
		add(previewDadDragHandle);

		if(previewTankmanDragHandle != null) remove(previewTankmanDragHandle);
		previewTankmanDragHandle = new FlxSprite();
		previewTankmanDragHandle.makeGraphic(120, 140, FlxColor.TRANSPARENT);
		previewTankmanDragHandle.scrollFactor.set();
		previewTankmanDragHandle.x = previewTankmanAnchorX;
		previewTankmanDragHandle.y = previewTankmanAnchorY;
		add(previewTankmanDragHandle);
	}

	function lockPreviewTransform(char:Character, fixedScale:Float, anchorX:Float, anchorY:Float):Void
	{
	...
	function previewNoteHit(column:Int, mustPress:Bool, ?noteType:String = ''):Void
	{
		...
		// GF Sing targets the girlfriend, which we don't have a preview for, and
		// No Animation means exactly that - so skip both.
		if(noteType == 'GF Sing' || noteType == 'No Animation') return;
